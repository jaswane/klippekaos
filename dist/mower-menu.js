...
function preview(canvas,id,card=false){const actor={x:0,y:0,angle:card?Math.PI/2:-Math.PI/2,speed:0,travel:0};KlippeMower.setAppearance(actor,id);previews.set(canvas,{actor,spec:KlippeMower.SPRITES[id],card});}
function paint(){for(const [canvas,{actor,spec,card}] of previews){if(!canvas.isConnected){previews.delete(canvas);continue;}if(!canvas.getClientRects().length)continue;
 // Card previews use their displayed aspect ratio; result previews retain their existing framing.
 if(card){const ratio=Math.min(root.devicePixelRatio||1,2),width=Math.round(canvas.clientWidth*ratio),height=Math.round(canvas.clientHeight*ratio);if(canvas.width!==width||canvas.height!==height){canvas.width=width;canvas.height=height;}}
...
function choices(container,profile,choose){container.replaceChildren();for(const p of Object.values(Klippe.mowerProfiles)){
 ...
 const image=document.createElement('canvas');image.width=240;image.height=130;image.className='mower-preview';image.setAttribute('role','img');image.setAttribute('aria-label',spec.name);preview(image,p.id,true);
