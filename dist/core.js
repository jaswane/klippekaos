...
const eventTuning={catRadius:16,waspFirstMin:12,waspFirstMax:18,waspCooldownMin:18,waspCooldownMax:26,waspMinDistance:90,waspMaxDistance:240,waspSeparation:80,waspPenalty:200,waspPenaltyCap:400};
...
const terrainTuning={heavySpeedFactor:.78,wildflowerPenalty:180};
...
function wildflowerAt(x,y,garden){const l=levelFor(garden);return inPatch(x,y,l.wildflowers)&&inPolygon(x,y,l.polygon)&&l.obstacles.every(o=>distance(x,y,o)>P.trim);}
...
class Game{
 ...
 reset(mode=this.mode||'normal',garden=this.garden||'garden1'){
  ...
  for(let y=0;y<this.ny;y++)for(let x=0;x<this.nx;x++)if(mowable((x+.5)*P.cell,(y+.5)*P.cell,this.garden)){this.mask[y*this.nx+x]=1;this.total++;if(inPatch((x+.5)*P.cell,(y+.5)*P.cell,this.level.heavy)){this.terrain[y*this.nx+x]=1;this.heavyTotal++;}}
 ...
 terrainLoad(){
  ...
  for(const offset of [-.65,0,.65]){const x=Math.floor((this.x+Math.cos(a+offset)*P.deck)/P.cell),y=Math.floor((this.y+Math.sin(a+offset)*P.deck)/P.cell),i=y*this.nx+x;if(x>=0&&x<this.nx&&y>=0&&y<this.ny&&this.mask[i]===1&&this.terrain[i])load++;}
  return load/3;
 }
 result(){
 ...
 updateCat(dt,events){
  const c=this.cat;if(!this.started||c.finished)return;
  if(this.time>=16.5&&!c.warned){c.warned=true;events.push('cat-warning');}
  if(this.time<18)return;c.active=true;
  ...
  if((c.x-r.from[0])*ux+(c.y-r.from[1])*uy>length){c.active=false;c.finished=true;events.push('cat-gone');}
 }
 spawnWasps(events){
 ...
 updateWasps(dt,events){
  ...
  if(!n.active)this.nextWaspAt=this.time+eventTuning.waspCooldownMin+this.random()*(eventTuning.waspCooldownMax-eventTuning.waspCooldownMin);
 }
...
root.Klippe={Game,P,obstacles,legal,distance,mowable,recordRun,recordKey,recordFlags,padInput,eventTuning,seededRandom,waspSpawnValid,catContact,levels,inPolygon,starterMower,terrainTuning,wildflowerAt,inPatch,catRoutes,seededRoute};if(typeof module!=='undefined')module.exports=root.Klippe;
