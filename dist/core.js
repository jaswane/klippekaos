...
const eventTuning={catRadius:16,catWarning:1.5,catFirst:18,catCooldown:13.5,hazardGap:3,waspWarning:.9,waspDuration:4.5,waspCooldown:6,waspMaxEvents:4,waspClearance:56,waspStartClearance:110,waspSeparation:130,waspPenalty:200,waspPenaltyCap:400};
...
// Placement is round-seeded and independent of event timing or the mower's current position.
...
function waspSpawnValid(x,y,game,placed=[]){
 const l=game.level,clear=eventTuning.waspClearance;
 return Number.isFinite(x)&&Number.isFinite(y)&&mowable(x,y,game.garden)&&legal(x,y,game.garden)&&
  Math.hypot(x-(l.start?.x??101),y-(l.start?.y??482))>=eventTuning.waspStartClearance&&
  placed.every(p=>Math.hypot(x-p.x,y-p.y)>=eventTuning.waspSeparation)&&
  l.polygon.every((p,i)=>{const q=l.polygon[(i+1)%l.polygon.length];return segmentDistance(x,y,...p,...q)>=clear;})&&
  l.obstacles.every(o=>distance(x,y,o)>=clear)&&
  (l.wildflowers||[]).every(p=>Math.hypot(x-p.x,y-p.y)>=Math.max(p.rx,p.ry)+clear);
}
function hiddenWaspNests(game){
 if(!game.mechanics.wasps)return [];
 let salt=0;for(const c of game.garden)salt=(Math.imul(salt,31)+c.charCodeAt(0))>>>0;
 const random=seededRandom((game.eventOptions.seed??0)^salt^0x7b014),count=2+Math.floor(random()*3),nests=[];
 for(let attempt=0;attempt<512&&nests.length<count;attempt++){
  const x=(Math.floor(random()*P.width/P.cell)+.5)*P.cell,y=(Math.floor(random()*P.height/P.cell)+.5)*P.cell;
  if(waspSpawnValid(x,y,game,nests))nests.push({id:nests.length,x,y,state:'hidden'});
 }
 return nests;
}
...
class Game{
 ...
 setMowerProfile(id){const profile=mowerProfiles[id];if(!profile)throw new Error('Unknown mower profile: '+id);Object.assign(this.mower,profile);}
 get mowRadius(){return P.deck*this.mower.mowWidthMultiplier;}
 ...
 mow(x,y){let fresh=0;const r=this.mowRadius,s=P.cell;
  for(let gy=Math.max(0,Math.floor((y-r)/s));gy<=Math.min(this.ny-1,Math.floor((y+r)/s));gy++)for(let gx=Math.max(0,Math.floor((x-r)/s));gx<=Math.min(this.nx-1,Math.floor((x+r)/s));gx++){
   ...
   if(this.mask[i]===1){this.mask[i]=2;this.cut++;if(this.terrain[i])this.heavyCut++;fresh++;this.recentCells.push(i);}else if(this.travel-this.last[i]>r*2.4){this.repeat++;}this.last[i]=this.travel;
  }return fresh;
 }
 terrainLoad(){
  if(!this.heavyTotal)return 0;let load=0;const a=this.angle+(this.speed<0?Math.PI:0),r=this.mowRadius;
  // Sample the incoming strip, not side points already erased by the previous deck pass.
  for(const offset of [-r*.5,0,r*.5]){const x=Math.floor((this.x+Math.cos(a)*(r+P.cell*2)-Math.sin(a)*offset)/P.cell),y=Math.floor((this.y+Math.sin(a)*(r+P.cell*2)+Math.cos(a)*offset)/P.cell),i=y*this.nx+x;if(x>=0&&x<this.nx&&y>=0&&y<this.ny&&this.mask[i]===1&&this.terrain[i])load++;}
  return load/3;
 }
 ...
 encounterWasps(ax,ay,bx,by){
  ...
  for(const n of this.waspNests){
   if(n.state!=='hidden')continue;
   const cell=Math.floor(n.y/P.cell)*this.nx+Math.floor(n.x/P.cell);
   if(this.mask[cell]===1&&segmentDistance(n.x,n.y,ax,ay,bx,by)<=this.mowRadius){n.state='queued';n.encounteredAt=this.time;}
  }
 }
 spawnWasps(events){
  if(!this.mechanics.wasps||!this.started||this.done||this.nest.active||this.waspSpawns.length>=eventTuning.waspMaxEvents||this.cat.active||(this.cat.warned&&!this.cat.finished)||this.time<this.hazardUntil||this.time<this.nextWaspAt)return false;
  const nest=this.waspNests.find(n=>n.state==='queued');if(!nest)return false;
  const {x,y,id}=nest;nest.state='used';this.nest={id,x,y,originX:x,originY:y,revealed:true,active:true,age:0,caught:false};this.waspSpawns.push({id,x,y,time:this.time});events.push('wasps');return true;
 }
 updateWasps(dt,events){
  if(!this.mechanics.wasps||!this.started)return;
  if(!this.nest.active&&this.spawnWasps(events))return; // Full warning duration starts on this frame.
  const n=this.nest;if(!n.active)return;
  const previousAge=n.age;n.age+=dt;
  // Integrate only the part after the warning, including frames that cross its boundary.
  const attackDt=Math.max(0,n.age-eventTuning.waspWarning)-Math.max(0,previousAge-eventTuning.waspWarning);
  if(attackDt>0){const vx=this.x-n.x,vy=this.y-n.y,d=Math.hypot(vx,vy),travel=Math.min(d,82*attackDt);n.x+=vx/(d||1)*travel;n.y+=vy/(d||1)*travel;if(Math.hypot(this.x-n.x,this.y-n.y)<20){n.caught=true;n.active=false;this.waspStings++;this.penalty=Math.min(eventTuning.waspPenaltyCap,this.penalty+eventTuning.waspPenalty);events.push('sting');}}
  if(n.age>=eventTuning.waspDuration&&n.active){n.active=false;events.push('escaped');}
  if(!n.active){this.hazardUntil=this.time+eventTuning.hazardGap;this.nextWaspAt=this.time+eventTuning.waspCooldown;}
 }
 ...
 step(dt,input={}){
  ...
  const target=throttle*(throttle>=0?P.maxSpeed:P.reverseSpeed)*speedFactor*terrainFactor*this.mower.speedMultiplier;
  ...
  let oldx=this.x,oldy=this.y;this.angle+=this.speed/P.wheelbase*Math.tan(this.steer*P.steerAngle)*turnFactor*this.mower.steeringMultiplier*dt;
  ...
  this.encounterWasps(oldx,oldy,this.x,this.y);
  ...
  if(this.mechanics.find&&!this.find.revealed&&fresh>0&&Math.hypot(this.x-this.find.x,this.y-this.find.y)<this.mowRadius+5){this.find.revealed=true;events.push('find');}
  ...
  return{fromX:oldx,fromY:oldy,x:this.x,y:this.y,moved:dist>.001,mowRadius:this.mowRadius,fresh,events,cells:this.recentCells};
 }
}
