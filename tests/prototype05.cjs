...
test('Career terrain and enabled events follow each level, including repeated wasps only on stage five',()=>{
 for(let i=2;i<=5;i++){const g=new K.Game('normal','career'+i);assert.equal(g.heavyTotal>0,i===2);assert.equal(g.flowerTotal>0,i===4);g.started=true;g.x=100;g.y=500;run(g,90);assert.equal(g.cat.finished,i===3);assert.equal(g.waspSpawns.length>1,i===5);}
});
...
test('Seeded combined runs never overlap dangers and preserve at least three seconds between them',()=>{
 for(let seed=0;seed<8;seed++){const g=new K.Game('normal','garden1',undefined,{seed,catRoute:'left-right'});g.started=true;g.x=100;g.y=500;let previous=null,lastEnd=-Infinity;for(let i=0;i<90*120;i++){g.step(1/120);const current=g.nest.active?'wasp':g.cat.active||(g.cat.warned&&!g.cat.finished)?'cat':null;assert(!(g.nest.active&&(g.cat.active||(g.cat.warned&&!g.cat.finished))));if(previous&&!current)lastEnd=g.time;if(current&&!previous)assert(g.time-lastEnd>=3-1/120);previous=current;}assert(g.cat.finished);assert(g.waspSpawns.length>=2);}
});
...
test('Summer garden is larger, distinct and offers an unobstructed central escape corridor',()=>{
 ...
 for(let seed=0;seed<20;seed++){const g=new K.Game('normal','career5',undefined,{seed});g.x=450;g.y=290;assert(g.spawnWasps([]));const p=g.waspSpawns[0];assert(K.legal(p.x,p.y,g.garden));assert(K.mowable(p.x,p.y,g.garden));}
});
